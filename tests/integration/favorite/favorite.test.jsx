...
const mockFavoriteServer = () => {
  server.use(
    http.get(apiUrl(ENDPOINTS.FAVORITE.LIST), () =>
      ok({
        content: favoriteIds.map((policyId) => ({
          policyId,
          status: 'INTEREST',
          savedAt: '2026-09-26',
          policy: toPolicySummary(POLICIES.find((policy) => policy.id === policyId)),
        })),
      }),
    ),
    http.post(apiUrl(ENDPOINTS.FAVORITE.DETAIL(':policyId')), ({ params }) => {
      requests.push(`POST ${params.policyId}`);
      favoriteIds = [...favoriteIds, Number(params.policyId)];
      return ok();
    }),
    http.delete(apiUrl(ENDPOINTS.FAVORITE.DETAIL(':policyId')), ({ params }) => {
